  ...
  if (option && option.query) {
    ...
    url = URL.format(
      Object.assign({}, urlObject, {
        query: Object.assign({}, urlObject.query, option.query),
      }),
    )
  }
  return url
...
class Fxios extends EventEmitter {
  constructor(config = exports.defaultRequestConfig) {
    ...
    const { base } = config,
      requestConfig = __rest(config, ['base'])
    ...
    this.get = (url, option, runtimeConfig) =>
      this.request('get', url, option, runtimeConfig)
    this.head = (url, option, runtimeConfig) =>
      this.request('head', url, option, runtimeConfig)
    this.post = (url, option, runtimeConfig) =>
      this.request('post', url, option, runtimeConfig)
    this.put = (url, option, runtimeConfig) =>
      this.request('put', url, option, runtimeConfig)
    this.patch = (url, option, runtimeConfig) =>
      this.request('patch', url, option, runtimeConfig)
    this.delete = (url, option, runtimeConfig) =>
      this.request('delete', url, option, runtimeConfig)
  }
